...
public class Reset extends Controller {
    ...
    public static Result runAsk() {
        ...
        Logger.debug("runAsk: compte = " + compte);

        // If we do not have this email address in the list, we should not expose this to the compte.
        // This exposes that the compte has an account, allowing a compte enumeration attack.
        ...
        if (compte == null) {
            Logger.debug("No compte found with email " + email);
            sendFailedPasswordResetAttempt(email);
            return ok(runAsk.render());
        }

        Logger.debug("Sending password reset link to compte " + compte);
    ...
    public static Result runReset(String token) {
        ...
        try {
            ...
            // check email
            Compte compte = Compte.find.byId(resetToken.userId);
            if (compte == null) {
     ...
     */
    private static void sendPasswordChanged(Compte compte) throws EmailException {
