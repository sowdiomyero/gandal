...
public class Signup extends Controller {
    ...
    public static Result save() {
        ...
        try {
            ...
            compte.email = register.email;
            compte.fullname = register.fullname;
            compte.passwordHash = Hash.createPassword(register.inputPassword);
     ...
     *
     * @param registerForm Compte Form submitted
    ...
    private static Result checkBeforeSave(Form<Application.Register> registerForm, String email) {
        // Check unique email
        if (Compte.findByEmail(email) != null) {
     ...
     *
     * @param compte compte created
     * @throws EmailException Exception when sending mail
     */
    private static void sendMailAskForConfirmation(Compte compte) throws EmailException, MalformedURLException {
    ...
    public static Result confirm(String token) {
        Compte compte = Compte.findByConfirmationToken(token);
        if (compte == null) {
        ...
        try {
            if (Compte.confirm(compte)) {
                sendMailConfirmation(compte);
                flash("success", Messages.get("account.successfully.validated"));
                return ok(confirm.render());
            } else {
                Logger.debug("Signup.confirm cannot confirm compte");
     ...
     * @throws EmailException Exception when sending mail
     */
    private static void sendMailConfirmation(Compte compte) throws EmailException {
