...
public class Token extends Model {
     ...
     */
    private static Token getNewToken(Compte compte, TypeToken type, String email) {
        Token token = new Token();
        token.token = UUID.randomUUID().toString();
        token.userId = compte.getId();
        token.type = type;
        token.email = email;
        token.save();
        return token;
     ...
     *
     * @param compte the current compte
     * @throws java.net.MalformedURLException if token is wrong.
     */
    public static void sendMailResetPassword(Compte compte) throws MalformedURLException {
        sendMail(compte, TypeToken.password, null);
    }
     ...
     * @throws java.net.MalformedURLException if token is wrong.
     */
    public static void sendMailChangeMail(Compte compte, @Nullable String email) throws MalformedURLException {
        sendMail(compte, TypeToken.email, email);
    }
     ...
     * @throws java.net.MalformedURLException if token is wrong.
     */
    private static void sendMail(Compte compte, TypeToken type, String email) throws MalformedURLException {

        Token token = getNewToken(compte, type, email);
